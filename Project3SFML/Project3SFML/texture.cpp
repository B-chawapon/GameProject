...
int main()
{
	...
	sf::Vector2f positionbg(screen.x/2, screen.y/2);
	...
	while (window.isOpen())
	{
		...
		{
			positionbg.x = wizardsp.getPosition().x+10;
		}
		else {
			positionbg.x = screen.x/2;
		}
		
		if (wizardsp.getPosition().y + 10 > screen.y/2 )
		{
			positionbg.y = wizardsp.getPosition().y + 10;
		}
		else {
			positionbg.y = screen.y / 2;
		}
